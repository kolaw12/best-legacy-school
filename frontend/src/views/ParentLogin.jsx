...
const ParentLogin = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (profile.role === 'parent' || ['super_admin', 'school_admin'].includes(profile.role)) {
                const target = from.startsWith('/portal') ? from : '/portal/dashboard';
                navigate(target);
            } else {
                setError(`Your role (${profile.role_display}) doesn't have access to the Parent Portal.`);
            }
        } catch (err) {
            setError(
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-secondary/5 via-white to-primary/5 py-16 px-4">
            <div className="w-full max-w-md">
                <div className="bg-white rounded-3xl shadow-card-lg p-8">
                    ...
                    <div className="text-center">
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-secondary-dark bg-secondary-soft px-3 py-1 rounded-full">
                            <span className="w-1.5 h-1.5 rounded-full bg-secondary" />
                            Parent Portal
                        </span>
                    </div>
                    <h1 className="mt-3 text-2xl font-black text-ink text-center">Welcome back.</h1>
                    <p className="mt-1 text-sm text-gray-500 text-center">Sign in to view your child's progress.</p>
                ...
                <div className="mt-6 text-center">
                    <Link to="/" className="text-xs text-gray-400 hover:text-ink transition">← Back to school website</Link>
                </div>
            </div>
        </div>
    );
